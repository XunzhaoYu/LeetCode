...
def compress(chars: List[str]) -> int:
    # 52 ms, faster than 89.04%. Same to the best solution from submissions (48 ms).
    write, cur_char, cur_count = 1, chars[0], 0
    for read_char in chars:
        if cur_char != read_char:
            if cur_count != 1:
                str_count = str(cur_count)
                for c in str_count:
                    chars[write] = c
                    write += 1
            chars[write] = read_char
            write += 1
            cur_count = 1
            cur_char = read_char
        else:
